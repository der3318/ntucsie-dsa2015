...
void push_id(char *_id, int _index)
{
	...
	ids[index].index.push_back(_index);
...
void find_reg_in_vector(std::vector<int>& ans, int index, char *_p)
{
	if(ids[index].index.empty())	return;
	for(int i = 0; i < (int)ids[index].index.size(); i++) 
	{
		if(data[ids[index].index[i]].cash != -1 && WildTextCompare(data[ids[index].index[i]].account, _p) && ids[index].index[i] != current_index)
			ans.push_back(ids[index].index[i]);
	}
}

bool alphabetsort(int i, int j) {
	if(strcmp(data[i].account, data[j].account) < 0)	return true;
	else return false;
}
...
void find(char* _p)
{
	...
	if(*_p != '*' && *_p != '?')
	{
		...
		find_reg_in_vector(ans, index, _p);
	}
	else	for(int i = 0 ; i < 62 ; i++)	find_reg_in_vector(ans, i, _p);
	std::sort(ans.begin(), ans.end(), alphabetsort);
	if(!ans.empty())	printf("%s", data[ans[0]].account);
	for(int i = 1; i < (int)ans.size(); i++)	printf(", %s", data[ans[i]].account);
	printf("\n");
}
