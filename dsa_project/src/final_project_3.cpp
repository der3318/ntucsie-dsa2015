...
{
	std::list<int> &r_1 = data[index_1].record, &r_2 = data[index_2].record, new_record;
	std::list<int>::iterator it_1 = r_1.begin(), it_2 = r_2.begin();
	while(it_1 != r_1.end() && it_2 != r_2.end())
	{
		if(*it_1 < *it_2)	new_record.push_back(*it_1++);
		else if (*it_1 > *it_2)
		{
			if(record[ *it_2 ].to == index_2)	record[ *it_2 ].to = index_1;
			else	record[ *it_2 ].from = index_1;
			new_record.push_back(*it_2++);
		}
		else
		{
			if(record[ *it_1 ].to == index_1)	record[ *it_1 ].from =  -1;
			else	record[ *it_1 ].to = -1;
			new_record.push_back(*it_1++);
			it_2++;
		}
	}
	while(it_1 != r_1.end())	new_record.push_back(*it_1++);
	while(it_2 != r_2.end())
	{
		if(record[ *it_2 ].to == index_2)	record[ *it_2 ].to = index_1;
		else	record[ *it_2 ].from = index_1;
...
{
	std::list<int> &r = data[current_index].record;
	int found = 0;
	...
	if(flag == current_index)
		while(it != r.end())
		{
			if(record[ *it ].to == -1)	{	printf("To %s %d\nFrom %s %d\n", id, record[ *it ].cash, id, record[ *it ].cash);	found = 1;	}
			else if(record[ *it ].from == -1)	{	printf("From %s %d\nTo %s %d\n", id, record[ *it ].cash, id, record[ *it ].cash);	found = 1;	}
			it++;
		}
	else
		while(it != r.end())
		{
			if(record[ *it ].to == flag)	{	printf("To %s %d\n", id, record[ *it ].cash);	found = 1;	}
			else if(record[ *it ].from == flag)	{	printf("From %s %d\n", id, record[ *it ].cash);	found = 1;	}
			it++;
		}
